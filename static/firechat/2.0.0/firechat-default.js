...
(function(Firebase) {
  ...
  Firechat.prototype = {
    ...
    _onNewMessage: function(roomId, snapshot) {
      if($("#firechat-container").is(':hidden'))
      {
        togglechat();
      }
  ...
  Firechat.prototype.createRoom = function(roomName, roomType, callback) {
    ...
    newRoomRef.set(newRoom, function(error) {
      if (!error) {
        ...
        self._roomRef.orderByChild("name").equalTo("ab0a81e8-2f0e-48f5-8c47-2b9f2c06b380_abc025dd-634a-4a26-8a9f-736d501f2e68").on("child_added", function(snapshot) {
    ...
    var self = this,
        ...
        renderPrivateInvitePrompt = function(event) {
          ...
          if (userId && userName) {
            ...
            $prompt.find('[data-toggle=accept]').first().click(function() {
              $prompt.remove();
              var roomName = self._user.id + "_" + userId;
                self._chat._roomRef.orderByChild("name").equalTo(roomName).once('value', function(snapshot) {
                var exists = (snapshot.val() !== null);
                    if (exists) {
                      ...
                      console.log(a);
                      self._chat.enterRoom(a);
                      self._chat.inviteUser(userId, a, roomName);
                  } else {
